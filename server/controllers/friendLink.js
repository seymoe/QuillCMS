...
export default {
  ...
  async createOne(req, res, next) {
    ...
    try {
      ...
      res.send(renderApiData(res, 200, '友情链接创建成功', { id: linkObj._id }))
    } catch (err) {
      res.status(500).send(renderApiErr(req, res, 500, err))
    }
  },
  ...
  async getOne(req, res, next) {
    try {
      ...
      res.send(renderApiData(res, 200, '获取成功', link || {}))
    } catch (err) {
      res.status(500).send(renderApiErr(req, res, 500, err))
    }
  },
  ...
  async getList(req, res, next) {
    try {
      ...
      res.send(renderApiData(res, 200, '友情链接列表获取成功', linkObj))
    } catch (err) {
  ...
  async updateOne(req, res, next) {
    ...
    try {
      ...
      res.send(renderApiData(res, 200, '友链更新成功', { id: item_id }))
    } catch (err) {
      res.status(500).send(renderApiErr(req, res, 500, err))
    }
  },
